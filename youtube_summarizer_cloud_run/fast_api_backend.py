...
def generate_summary_content(youtube_link: str, model_name: str, additional_prompt: str | None):
    '''
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid YouTube link or URI issue: {e}")
    ...
    contents = [
        youtube_video,
        types.Part.from_text(text=prompt_text), # Using the potentially modified prompt_text
    ]
    if additional_prompt.strip(): # Add additional_prompt part only if it's not just whitespace
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"GenAI content generation failed: {e}")
