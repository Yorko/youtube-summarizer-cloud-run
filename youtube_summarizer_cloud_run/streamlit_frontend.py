...
def summarize_video_with_backend(video_url, model_name, custom_prompt):
    ...
    try:
        ...
        response = requests.post(
            summarize_endpoint, data=payload, timeout=300
        ...
        if response.status_code == 200:
            return response.json()[
                "summary"
            ], None  # Fast API app returns plain text summary
        else:
            try:
                # Try to parse error from Fast API if it sends JSON
                error_details = response.json()
                error_message = (
                    f"Error from backend: {error_details.get('error', response.text)}"
                )
        ...
        return (
            None,
            f"Could not connect to the summarization backend: {e}. Ensure the Fast API app is running at {BACKEND_URL}.",
        )
...
with st.container():
    youtube_link = st.text_input(
        "🔗 YouTube Video Link:", placeholder="e.g., youtube.com/watch?v="
    )
    ...
    custom_instructions = st.text_area(
        ...
        placeholder="e.g., Summarize for a beginner, focus on the technical aspects, provide a 3-bullet point summary...",
    )
...
if summarize_button:
    if not youtube_link:
        ...
        with st.spinner(
            f"🔄 Requesting summary from backend ({selected_model})... This may take a few minutes for longer videos."
        ):
            summary, error_msg = summarize_video_with_backend(
                youtube_link, selected_model, custom_instructions
            )
            ...
            if error_msg:
                ...
                st.warning(
                    "⚠️ No summary could be generated, or an unknown error occurred."
                )

st.markdown("---")
st.markdown(
    "Built with ❤️ using [Streamlit](https://streamlit.io). Based on [this](https://codelabs.developers.google.com/devsite/codelabs/build-youtube-summarizer) codelab."
)
